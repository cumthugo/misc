...
def get_calibration_array(text):
    ret = []
    for i in range(0x20):
        p ='result\[' + '%02X'%i + '\]=(0x[0-9A-F]*)'
        ret.append(text_find(p,text))
    return ret

def get_read_cal(text):
    read_cal_text = get_text(text,'Starting Read calibration','Byte 0')
    return get_calibration_array(read_cal_text)
    
def get_write_cal(text):
    write_cal_text = get_text(text,'Starting Write calibration','Byte 0')
    return get_calibration_array(write_cal_text)
...
#return BYTE 0-3 Start and End value
def get_byte(text):
    ret = []
    ...
    for i in range(4):
        ...
        ret.append([calc_hc_abs_value(start_hc,start_abs),calc_hc_abs_value(end_hc,end_abs)])
    return ret
...
def check_file(file_path):
    try:
        ...
        byte = get_byte(t)
        read_cat = get_read_cal(t)
        write_cat = get_write_cal(t)
        return True
    except:
        return False
...
def calc_DQS_cals(bytes):
    ...
    return get_DQS(new_byte)
 
def calc_calibration_start_end(cal_arr,index):
    s=0;e=0
    last_value = '1'
...
def calc_calibration(cals_arr):
    ret = 0
    for i in range(4):
        start_arr = []; end_arr =[]
        for cal in cals_arr:
            start,end = calc_calibration_start_end(cal,i)
...
def calc_all_calibration_param(file_texts):
    ret = []
    ctrl_list = []
    byte_list = []
    ...
    for file_text in file_texts:
        ...
        read_cal_list.append(get_read_cal(file_text))
        write_cal_list.append(get_write_cal(file_text))
    ret.extend(calc_MPWLDECTRL(ctrl_list))
    ret.extend(calc_DQS_cals(byte_list))
    ret.append(calc_calibration(read_cal_list))
    ret.append(calc_calibration(write_cal_list))
    return ret
