...
wb = load_workbook(filename=r'k256.xlsx')
ws = wb.get_sheet_by_name('k256')

rows = ws.rows
cols = ws.columns

priority_str = {'High':'高','Medium': '普通', 'Low':'低','Unassigned':'普通'}
serverity_str = {'High':'P1','Medium':'P2','Low':'P3','Unassigned':'普通'}
version_str = {u'1':'328_YF',u'2':'K211_YF',u'3':'K216',u'4':'K256',u'5':'K215'}

w_file = open('issues_write.csv','wb') 
...
for i in range(2,len(rows)+1):
    ...
    item = ['','HMI12','ST Bug','新建',priority_str[v_priority],summary,serverity_str[serverity],foundin,'YFVE_Z 张勇','Ver*.***','rev.0000; rev.0000','Ver*.***','HMI',defectid,version,'否','%s\n\nComments:\n%s'%(description,comment)]
